...
func _ready():
	...
	beat_timer.wait_time = time_from_tempo($MetroDial/DialNode/DialDisplay.tempo, subdivisions)
...
func _process(delta):
	if playing:
		...
		var time = time_from_tempo($MetroDial/DialNode/DialDisplay.tempo, subdivisions)
		print(str(time))
